...
def get_predictions(train_ratio,y,aic_min_order,bic_min_order,mse_min_order):
    err_aic = []
    err_bic = []
    err_mse = []
    for i in range(3):
        ...
        print('MODEL AIC')
        (p,q) = aic_min_order[i]
        f,ci,e = out_of_sample_prediction(p=p, q=q, y_true=y, train_ratio=train_ratio[i],signif= False)
        err_aic.append(e)
...
def get_predictions2(train_ratio,z,p,aic_min_order,bic_min_order,mse_min_order,plot = False):
    err_aic = []
    err_bic = []
    err_mse = []
    for i in range(3):
        ...
        print('MODEL AIC')
        (p,q) = aic_min_order[i]
        e = p_with_fit_of_z(p=p,q=q,z_true = z ,p_true =p, train_ratio=train_ratio[i], signif = True)
        err_aic.append(e)
